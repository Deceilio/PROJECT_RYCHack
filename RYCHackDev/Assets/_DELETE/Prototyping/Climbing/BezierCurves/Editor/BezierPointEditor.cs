...
public class BezierPointEditor : Editor
{
    ...
    void OnSceneGUI()
    {
        ...
        var fmh_97_70_638269402718487923 = point.transform.rotation; Vector3 newPosition = Handles.FreeMoveHandle(point.position, HandleUtility.GetHandleSize(point.position) * 0.2f, Vector3.zero, Handles.CubeHandleCap);
    ...
    private static void HandleConnected(BezierPoint p)
    {
        ...
        var fmh_113_70_638269402718525920 = p.transform.rotation; Vector3 newGlobal1 = Handles.FreeMoveHandle(p.globalHandle1, HandleUtility.GetHandleSize(p.globalHandle1) * 0.15f, Vector3.zero, Handles.SphereHandleCap);
        ...
        var fmh_122_70_638269402718529796 = p.transform.rotation; Vector3 newGlobal2 = Handles.FreeMoveHandle(p.globalHandle2, HandleUtility.GetHandleSize(p.globalHandle2) * 0.15f, Vector3.zero, Handles.SphereHandleCap);
    ...
    private static void HandleBroken(BezierPoint p)
    {
        ...
        var fmh_136_70_638269402718533735 = Quaternion.identity; Vector3 newGlobal1 = Handles.FreeMoveHandle(p.globalHandle1, HandleUtility.GetHandleSize(p.globalHandle1) * 0.15f, Vector3.zero, Handles.SphereHandleCap);
        var fmh_137_70_638269402718537011 = Quaternion.identity; Vector3 newGlobal2 = Handles.FreeMoveHandle(p.globalHandle2, HandleUtility.GetHandleSize(p.globalHandle2) * 0.15f, Vector3.zero, Handles.SphereHandleCap);
