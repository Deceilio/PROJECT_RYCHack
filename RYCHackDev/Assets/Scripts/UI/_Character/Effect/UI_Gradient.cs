...
{
    [AddComponentMenu("Psychain/Effects/UI Gradient")]
    public class UI_Gradient : BaseMeshEffect
    {
        [SerializeField]
        Type _gradientType; // Type of the gradient

        [SerializeField]
        Blend _blendMode = Blend.Multiply; // Blend mode of the gradient

        [SerializeField]
        bool _modifyVertices = true; // Checks if the gradient should modify vertices or not

        [SerializeField]
        [Range(-1, 1)]
        float _offset = 0f; // Checks the gradient offset

        [SerializeField]
        [Range(0.1f, 10)]
        float _zoom = 1f; // Checks the graduent zoom
