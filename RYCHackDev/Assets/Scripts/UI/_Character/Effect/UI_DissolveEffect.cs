...
namespace Deceilio.Psychain
{
    ...
    [AddComponentMenu("Psychain/Rendering/UI Dissolve Effect")]
    public class UI_DissolveEffect : BaseMeshEffect
    {
        public const string shaderName = "Psychain/Dissolve Effect"; // Name of the shader
        ...
        [SerializeField] [Range(0, 1)] float m_Location = 0.5f; // Shader location
        [SerializeField] [Range(0, 1)] float m_Width = 0.5f; // Shader width
        [SerializeField] [Range(0, 1)] float m_Softness = 0.5f; // Shader softness
        [SerializeField] [ColorUsage(false)] Color m_Color = new Color(0f, 0f, 0f); // Shader color
        [SerializeField] [HideInInspector] ColorMode m_ColorMode = ColorMode.Add; // Shader color mode
        [SerializeField] Material m_EffectMaterial; // Shader material
        [Range(0.25f, 3)] public float animationSpeed = 25; // Shader animation's speed
        public bool mainPanelMode; // Checks if the shader's main panel mode is enable or not

        new public Graphic graphic { get { return base.graphic; } } // Shader graphic
        public float location { get { return m_Location; } set { m_Location = Mathf.Clamp(value, 0, 1); _SetDirty(); } } // Shader location
        public float width { get { return m_Width; } set { m_Width = Mathf.Clamp(value, 0, 1); _SetDirty(); } } // Shader width
        public float softness { get { return m_Softness; } set { m_Softness = Mathf.Clamp(value, 0, 1); _SetDirty(); } } // Shader softness
        public Color color { get { return m_Color; } set { m_Color = value; _SetDirty(); } } // Shader color
        public ColorMode colorMode { get { return m_ColorMode; } } // Shader color mode
        public virtual Material effectMaterial { get { return m_EffectMaterial; } } // Shader material
        ...
        public static string GetDefaultMaterialPath(Shader shader)
        {
            ...
            return AssetDatabase.FindAssets("t:Material " + name)
                ...
                ?? ("Assets/ArtResources/UI/Materials/" + name + ".mat");
        }
