...
{
    public class BASIC_IdleState : WRLD_STATE
    {
            ...
            for (int i = 0; i < colliders.Length; i++)
            {
                ...
                if (targetCharacter != null)
                {
                    if (targetCharacter.characterStatsManager.entityTeamIDNumber != aiCharacter.aiCharacterStatsManager.entityTeamIDNumber)
                    {
                        ...
                        {
                            // BELOW CODE: If the A.I's potential target has an obstruction in between itself and the A.I, it will not add player as current target
