...
{
    public class ADVANCE_CohortPursueTargetStateHumanoid : WRLD_STATE
    {
        [Header("COMBAT STANCE STATE")]
        ADVANCE_CohortCombatStanceStateHumanoid combatStanceState; // Reference to the Combat Stance State script
        [Header("FOLLOW STATE")]
        ...
        public override WRLD_STATE Tick(AICharacterManager aiCharacter)
        {
            ...
            if (aiCharacter.distanceFromTarget > aiCharacter.maxDistanceFromCohort)
            {
                aiCharacter.animator.SetFloat("Vertical", 1, 0.1f, Time.deltaTime);
            }

            if (aiCharacter.distanceFromTarget <= aiCharacter.returnDistanceFromCohort)
            {
